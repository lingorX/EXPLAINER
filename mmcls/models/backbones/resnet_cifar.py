...
#        self.proj = ProjectionHead(norm_cfg=self.norm_cfg)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.out_indices:
                outs.append(x)
#        outs[-1] = self.proj(outs[-1])
        return tuple(outs)

class ProjectionHead(nn.Module):
    def __init__(self, dim_in=2048, norm_cfg=None, proj_dim=128, proj='convmodule'):
        ...
        elif proj == 'convmodule':
            self.proj = nn.Sequential(
                nn.Conv2d(dim_in, proj_dim, kernel_size=1),
                build_norm_layer(norm_cfg, proj_dim)[1],
#                 nn.ReLU(inplace=True),
            )
