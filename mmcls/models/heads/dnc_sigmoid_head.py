...
class DNCSigmoidHead(MultiLabelClsHead):
    def __init__(self,
                 num_classes,
                 in_channels=128,
                 loss=dict(
                     ...
                     loss_weight=1.0),
                 init_cfg=None):
        super(DNCSigmoidHead, self).__init__(loss=loss, init_cfg=init_cfg)
    ...
    def forward_train(self, x, gt_label, **kwargs):
        x = self.pre_logits(x)
        gt_label = F.one_hot(gt_label, self.num_class_low)
        new_gt = gt_label
    ...
    def forward(self, x, img_metas=None, gt_label=None):          
        ...
        if gt_label is not None: 
            ...
            if self.proto_contrast_loss == True: 
